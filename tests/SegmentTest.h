...
//  nsol::Section sec;
  nsol::SegmentPtr seg1, seg2;

  
  nsol::NodePtr node1, node2;
...
#define RADIUS2 20.0f
  ...
  SegmentTest() 
    : v1( 1.0, 2.0, 3.0 )
    , v2( 4.0, 5.0, 6.0 )
    , seg1( nsol::SegmentPtr( new nsol::Segment ))
    , seg2( nsol::SegmentPtr( new nsol::Segment ))
    , node1( nsol::NodePtr( new nsol::Node( v1, 0, RADIUS1 )))
    , node2( nsol::NodePtr( new nsol::Node( v2, 0, RADIUS2 )))
  {
...
TEST_F(SegmentTest, pointers) {
  ...
  // Test next and prev
  ASSERT_EQ( seg1->prev( ), nsol::SegmentPtr( nullptr ));
  ASSERT_EQ( seg1->next( ), seg2 );
  ASSERT_EQ( seg2->prev( ), seg1 );
  ASSERT_EQ( seg2->next( ), nsol::SegmentPtr( nullptr ));

  //TODO
  //ASSERT_EQ(seg1.parentSection(), nullptr);

  // Test begin and end nodes 
  ASSERT_EQ(seg1->begin(), nsol::NodePtr( nullptr ));
  ASSERT_EQ(seg1->end(), nsol::NodePtr( nullptr ));
